...
export const NodeHandler = (opts) => {
    ...
    //       relative path.
    const target = path.join(artifact, absolutePathToRelativePath(opts.srcPath), path.dirname(file), base + ".js");
    const config = {
    ...
    return {
        shouldBuild: (files) => {
            ...
            if (!existing)
                return true;
            const result = files
                .map((x) => path.relative(process.cwd(), x).split(path.sep).join(path.posix.sep))
      ...
      `;
        ...
        watcher: {
            include: ["**/*.ts", "**/*.tsx", "**/*.js", "**/*.jsx"].map((glob) => path.resolve(path.join(opts.srcPath, glob))),
            ignore: [],
        },
    };
};
